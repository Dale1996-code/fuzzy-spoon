...
const safetyCategories = [
  ...
  },
  {
    ...
    items: ["Only approved box cutters", "Blades retracted while walking"],
  },
  {
...
export default function ShiftControlDashboard() {
  ...
  const getFreightStrategy = () => {
    ...
    const palletsPerPerson = totalAssociates
      ? shiftData.palletCount / totalAssociates
      : 0;
    ...
    if (shiftData.palletCount >= 15) {
      return {
        status: "HEAVY LOAD",
        strategy:
          "All hands on freight first 2 hours. Prioritize high-velocity aisles. Consider pulling 1 fresh associate for backup if critical.",
        pace: `${palletsPerPerson.toFixed(1)} pallets/person. Target: 1 pallet/45min.`,
      };
    }

    if (shiftData.palletCount >= 10) {
      return {
        status: "MODERATE LOAD",
        strategy:
          "Standard freight flow. Focus grocery team on freight while fresh maintains departments. Check progress at 2-hour mark.",
    ...
    return {
      status: "LIGHT LOAD",
      strategy:
        "Freight completion + deep zoning. Plan for topstock work and endcap maintenance after freight clear.",
  ...
  const getFreshWalkPriorities = () => {
    ...
    if (shiftData.problemAreas.includes("Deli cooking prep behind")) {
      priorities.push({
        ...
        priority: "HIGH",
        action:
          "Check hot case levels. Verify cook schedule on track. Ensure rotation happening.",
      });
    } else {
      priorities.push({
        area: "DELI/BAKERY",
    ...
    if (
      shiftData.problemAreas.includes("Produce not maxed/full") ||
      ...
      priorities.push({
        ...
        priority: "HIGH",
        action:
          "Full department audit. Check dates, rotation, culling. Fill all holes. Verify temp logs.",
      });
    } else {
      priorities.push({
        area: "PRODUCE/MEAT",
    ...
    if (shiftData.problemAreas.includes("Frozen/Dairy daily tasks")) {
      priorities.push({
        ...
        priority: "HIGH",
        action:
          "Verify daily task completion: milk rotation, yogurt dates, frozen door temps, ice cream leveling.",
      });
    } else {
      priorities.push({
        area: "FROZEN/DAIRY",
  ...
  const totalAssociates =
    shiftData.groceryStockers +
    shiftData.produceWorker +
    shiftData.meatWorker +
    shiftData.frozenDairyWorker +
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    saveData({
                      ...shiftData,
                      groceryStockers: parseInt(e.target.value, 10) || 0,
                    })
                  }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      saveData({
                        ...shiftData,
                        produceWorker: parseInt(e.target.value, 10) || 0,
                      })
                    }
                    className="w-full bg-gray-700 rounded px-3 py-2"
                    min="0"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      saveData({
                        ...shiftData,
                        meatWorker: parseInt(e.target.value, 10) || 0,
                      })
                    }
                    className="w-full bg-gray-700 rounded px-3 py-2"
                    min="0"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      saveData({
                        ...shiftData,
                        frozenDairyWorker: parseInt(e.target.value, 10) || 0,
                      })
                    }
                    className="w-full bg-gray-700 rounded px-3 py-2"
                    min="0"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      saveData({
                        ...shiftData,
                        deliBakeryWorkers: parseInt(e.target.value, 10) || 0,
                      })
                    }
                    className="w-full bg-gray-700 rounded px-3 py-2"
                    min="0"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  saveData({
                    ...shiftData,
                    palletCount: parseInt(e.target.value, 10) || 0,
                  })
                }
                className="w-full bg-gray-700 rounded px-3 py-2"
                min="0"
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-gray-800 rounded-lg p-4 mb-4 border-l-4 border-blue-500">
            ...
            <div className="text-lg font-semibold">
              {totalAssociates} Associates · {shiftData.palletCount} Pallets
            </div>
          </div>
            ...
            <div className="mb-2">
              <span
                className={`inline-block px-3 py-1 rounded text-sm font-bold ${
                  freightStrategy.status === "HEAVY LOAD"
                    ? "bg-red-600"
                    : freightStrategy.status === "MODERATE LOAD"
                      ? "bg-yellow-600"
                      : "bg-green-600"
                }`}
              >
              ...
              <ul className="space-y-2">
                {shiftData.problemAreas.map((area, idx) => (
                  <li key={idx} className="flex items-start">
                    <AlertTriangle
                      className="mr-2 mt-1 flex-shrink-0 text-red-400"
                      size={16}
                    />
  ...
  if (currentView === "safety") {
    ...
    const totalItems = safetyCategories.reduce(
